...
import { EditorTabs, FilterTabs, DownloadTab, DecalTypes } from "../config/constants";
            ...
            {!snap.intro &&
                <>
                    <motion.div key="custom"
                        ...
                        <div className="flex items-center min-h-screen">
                            <div className="editortabs-container tabs">
                                {EditorTabs.map((tab) => <Tab key={tab.name}
                                    tab={tab}
                                    handleClick={() => {
                                        activeEditorTab == tab.name ? setActiveEditorTab("") :
                    ...
                    <motion.div className="filtertabs-container" {...slideAnimation("up")}>
                        {FilterTabs.map((tab) => <Tab key={tab.name}
                            tab={tab}
                            isFilterTab
                            isActiveTab={activeFilterTab[tab.name]}
                            handleClick={() => { handleActiveFilterTab(tab.name) }} />
                        )}
                        <Tab key="download tab"
                            isFilterTab
                            tab={DownloadTab}
